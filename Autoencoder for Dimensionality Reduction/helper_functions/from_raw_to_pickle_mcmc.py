...
def get_pickle(labelled):
    ...
    if labelled:
        ...
        names = []
        f = open("helper_files/labels_mcmc.txt", 'r')
        for line in f:
            l = line.split(" ")
            from_to = l[0].split("-")
        ...
        pkl_file = open("pkl/mcmc_labelled_data.pkl", 'wb')
        pickle.dump(labelled_data, pkl_file, protocol=4)
        pkl_file.close

        pkl_file = open("pkl/mcmc_labelled_names.pkl", 'wb')
        pickle.dump(names, pkl_file, protocol=4)
        pkl_file.close
    else: 
        # save all data

        pkl_file = open("pkl/mcmc.pkl", 'wb')
        pickle.dump(data, pkl_file, protocol=4)
        pkl_file.close

        pkl_file = open("pkl/mcmc.pkl", 'rb')
        data = []
